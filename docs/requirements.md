...
- FR-003: The Connect dashboard must connect, refresh, and disconnect Judge.me credentials without exposing the raw token after save.
- FR-004: Judge.me tokens must be encrypted before persistence and masked in UI/data views.
- FR-005: The Reviews queue must sync Judge.me reviews into `ReviewDraft` records scoped by shop.
...
- Security: OAuth tokens and Judge.me tokens must stay server-side; secrets must be env vars only.
- Privacy: customer/review content is merchant data and must be minimized, scoped by shop, and deletable.
- Reliability: AI/Judge.me/Shopify failures must return recoverable UI errors, not stack traces.
