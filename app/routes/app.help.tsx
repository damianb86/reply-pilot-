  ...
  if (intent === "privacy-data-request") {
    try {
      const [
        ...
        db.reviewProviderConnection.count({ where: { shop: session.shop } }),
      ...
      await sendContactEmail({
        ...
        message: [
          ...
          `- Legacy Judge.me connections: ${judgeMeConnections}`,
          `- Additional review provider connections: ${reviewProviderConnections}`,
  ...
  if (intent === "privacy-data-delete") {
    try {
      await Promise.all([
        ...
        db.reviewProviderConnection.deleteMany({ where: { shop: session.shop } }),
